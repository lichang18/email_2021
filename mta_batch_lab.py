from all_in_one import mta_predict, mta_header
from classify_user import mta_classify
import os
import pandas as pd

data_path = "./data/mta2"
result_path = "./result/mta2_with_cls_thd"
if not os.path.exists(result_path):
    os.mkdir(result_path)
    
list1, list2, list3, list4 = mta_classify(filepath=data_path)

def sort_file(file_list):
    files = dict()
    for file in file_list:
        df = pd.read_csv(os.path.join(data_path, file), encoding="utf-8", header=None, sep=";", names=mta_header, index_col=False)
...
def run_detector(sorted_file_list: list, list_name: str):
    ...
    while len(sorted_file_list) > 0:
        ...
        if len(sorted_file_list) > 0:
            ...
            sorted_file_list = sorted_file_list[:-3]
        print("{}.csv".format(i))
        mta_predict(account_list=account_list, result_path=os.path.join(write_path, "{}.csv".format(i)), data_path=data_path)
