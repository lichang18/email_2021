from all_in_one import imap_predict, imap_header
from classify_user import imap_classify
import os
import pandas as pd

data_path = "./data/imap"
result_path = "./result/imap_with_cls_thd"
if not os.path.exists(result_path):
    os.mkdir(result_path)
    
list1, list2, list3, list4 = imap_classify(filepath=data_path)

def sort_file(file_list):
    files = dict()
    for file in file_list:
        df = pd.read_csv(os.path.join(data_path, file), encoding="utf-8", header=None, sep=";", names=imap_header, index_col=False)
...
def run_detector(sorted_file_list: list, list_name: str):
    ...
    while len(sorted_file_list) > 0:
        ...
        if len(sorted_file_list) > 0:
            ...
            sorted_file_list = sorted_file_list[:-3]
        print("{}.csv".format(i))
        imap_predict(account_list=account_list, result_path=os.path.join(write_path, "{}.csv".format(i)), data_path=data_path)
